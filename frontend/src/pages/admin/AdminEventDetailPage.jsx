...
export const AdminEventDetailPage = () => {
  ...
  return <div className="flex flex-col gap-5"><div className="flex flex-wrap items-start justify-between gap-3"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h1 className="break-words text-2xl font-semibold">{event.name}</h1><Badge variant="outline">{event.status}</Badge></div><p className="text-sm text-muted-foreground">{event.description || 'No description'}</p></div><div className="flex flex-wrap gap-2"><Button variant="outline" onClick={() => navigate(`/admin/events/${id}/edit`)}><Pencil className="size-4" /> Edit</Button><Button asChild><Link to="/admin/qr-codes">All QR codes</Link></Button></div></div><div className="grid gap-4 md:grid-cols-2"><section className="glass-card rounded-2xl p-5"><h2 className="mb-3 font-medium">Event details</h2><dl className="grid gap-2 text-sm"><div>Event: {formatDateTime(event.startDate)} — {formatDateTime(event.endDate)}</div><div>Registration: {event.registrationStartDate ? formatDateTime(event.registrationStartDate) : 'Open now'} — {event.registrationEndDate ? formatDateTime(event.registrationEndDate) : 'No close date'}</div><div>Capacity: {event.capacity || 'Unlimited'}</div><div>Venue: {event.venue?.name || '—'} {event.venue?.address ? `· ${event.venue.address}` : ''}</div></dl></section><section className="glass-card flex flex-col items-center gap-4 rounded-2xl p-5"><h2 className="font-medium">Registration QR code</h2>{qr ? <><img src={qr.qrImage || qr.image} alt={`Registration QR for ${event.name}`} className="size-48 rounded-xl bg-white p-3 sm:size-64" /><p className="break-all text-center text-xs text-muted-foreground">{qr.qrUrl || qr.url}</p><div className="flex flex-wrap justify-center gap-2"><Button variant="outline" onClick={download}><Download className="size-4" /> Download</Button><Button variant="outline" onClick={regenerateCode} disabled={regenerate.isPending}><RefreshCw className="size-4" /> Regenerate</Button></div></> : <p className="text-sm text-muted-foreground">No QR code found.</p>}</section></div></div>;
};
